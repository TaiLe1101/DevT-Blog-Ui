...
function About() {
    return (
        <div className={`${cx('about')} ${cxm('section')}`} id="about">
            <h2 className={`${cxm('section__title')}`}>Về Tôi</h2>
            <span className={`${cxm('section__subtitle')}`}>
                Giới thiệu về tôi
            </span>

            <div
                className={`${cx('about__container')} ${cxm(
                    'container',
                    'grid'
                )}`}
            >
                <img
                    src={aboutImg}
                    alt="about img"
                    className={cx('about__img')}
                />

                <div className={cx('about__data')}>
                    <Info></Info>
                    <p className={cx('about__description')}>
                        Tôi là một lập trình viên FullStack, tôi có thể tạo
                        website với nhiều thư viện khác nhau, tôi có 1 năm kinh
                        nghiệm làm việc trong lĩnh vực này và nhận lại được rất
                        nhiều phản hồi tích cực về những dự án tôi đã làm.
                    </p>
